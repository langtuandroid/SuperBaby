...
#define BN_PLIST_FILE                  "res/stage032/HY-GK08-IT.plist"              // BN的plist 文件
#define BN_PNG_FILE                    "res/stage032/HY-GK08-IT.png"               // BN的 png 文件
#define NPC_GAF_CONFIG_FILE             "res/stage032/prawn.json"
#define NPC_GAF_FILE                    "res/stage032/HY-GK08-1-MS/HY-GK08-1-MS.gaf"    // NPC gaf
#define NPC_KEY                         "NPC-089"
#define NPC_COLLECT_DURATION            1.5f
#define NPC_ZORDER                      2
#define NPC_POS                         Vec2(1601,328)
 ...
 *  constructor
 */
Round032_01::Round032_01(){
    m_answersPos[0] = Vec2(752, 389);
    m_answersPos[1] = Vec2(1102, 389);
    m_answersPos[2] = Vec2(1432, 389);
}
...
//
Round032_01* Round032_01::createWithRoundIndex(const int roundIndex, StageScene* pStageScene, const Vec2& pos)
{
    auto pRound = new Round032_01();
 ...
 *  初始化
 */
bool Round032_01::initWithRoundIndex(const int roundIndex, StageScene* pStageScene, const Vec2& pos)
{
    ...
    for (unsigned long i = 0; i < 3; i++) {
        sprintf(buffer, "HY-GK08-1-DA%02ld.png",i+1);
...
void Round032_01::onEnterRound()
{
    ...
    this->schedule(CC_SCHEDULE_SELECTOR(Round032_01::changeScaleAnswersSchedule), 0.4f, -1, 0);
}

//提示答案
void Round032_01::changeScaleAnswersSchedule(float)
{
    if (m_InitAniIndex >= 3) {
        unschedule(CC_SCHEDULE_SELECTOR(Round032_01::changeScaleAnswersSchedule));
        m_jumped = true;
        if (m_opentouched) {
            openTouch();
        }
...
void Round032_01::changeActor()
{
    m_pNpcActor->playWithSequenceAliasName(TRIGGER);
}

void Round032_01::fadeAnswer()
{
    for (unsigned int i = 0; i < 3; i++) {
...
//
void Round032_01::celebrate(const float delay)
{
    if (m_celebrateCB) {
        m_celebrateCB(true);
    }
...
/*  处理npc收集 */
void Round032_01::processNpcCollect()
{
    auto npcBagPos = m_pStageScene->getRoundUINode()->getNpcBagPos();
    auto npcNewBafPos = Vec2(npcBagPos.x-118, npcBagPos.y-111);
    auto npcActions = Sequence::create(Spawn::create(JumpBy::create(0.5f, Vec2(0, 200), 100, 1),ScaleTo::create(0.5f, 1.1f), NULL),Spawn::create(MoveTo::create(0.5f, npcNewBafPos),ScaleTo::create(0.5f, 0.2f), NULL),CallFunc::create([this]{
