...
#define BN_PLIST_FILE                  "res/stage033/HY-GK09-IT.plist"              // BN的plist 文件
#define BN_PNG_FILE                    "res/stage033/HY-GK09-IT.png"               // BN的 png 文件
 ...
 *  constructor
 */
Round033_01::Round033_01()
{
    
    m_answersPos[0] = Vec2(836, 401);
    m_answersPos[1] = Vec2(1316, 401);
    m_answersPos[2] = Vec2(1796, 401);
}
...
//
Round033_01* Round033_01::createWithRoundIndex(const int roundIndex, StageScene* pStageScene, const Vec2& pos)
{
    auto pRound = new Round033_01();
 ...
 *  初始化
 */
bool Round033_01::initWithRoundIndex(const int roundIndex, StageScene* pStageScene, const Vec2& pos)
{
    ...
    for (unsigned long i = 0; i < 3; i++) {
        sprintf(buffer, "HY-GK09-1-DA%02ld.png",i+1);
        ...
        m_pAnswersSp[i]->setAnchorPoint(Vec2(0.5f,0.0f));
    }
...
void Round033_01::onEnterRound()
{
    ...
    this->schedule(CC_SCHEDULE_SELECTOR(Round033_01::changeScaleAnswersSchedule), 1.2f, -1, 0);   //
    
}

//改变答案大小以提示
void Round033_01::changeScaleAnswersSchedule(float)
{
    if (m_InitAniIndex >= 3) {
        unschedule(CC_SCHEDULE_SELECTOR(Round033_01::changeScaleAnswersSchedule));
        m_jumped = true;
        if (m_opentouched) {
            openTouch();
        }
 ...
 *  庆祝
 */
void Round033_01::celebrate(const float celebrateDelay /*庆祝延时*/ )
{
    if (m_celebrateCB) {
        m_celebrateCB(1);
    }
